...
class CTRL_module():
    ...
    def train(self, time_step):
        ...
        # self.wandb_imgs = sample_imgs(next_observations.detach().cpu().numpy(), controllable_effect.detach().cpu().numpy(), size=5)
...
def combine_frames(main_frame, mask, action, event):
    gray_mask = color.rgb2gray(mask.transpose(1,2,0))
    # bin_mask = gray_mask.round()
    fig, (ax1, ax2) = plt.subplots(1,2)
    ax1.imshow(main_frame.transpose(1,2,0))
    ax1.axis('off')
    # pdb.set_trace()
